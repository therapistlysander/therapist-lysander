...
<main id="main-content">
  ...
  <div class="page-hero">
    <div class="container--narrow">
      <span class="page-hero__eyebrow">{{ $hero?->content['subheading'] ?? 'Get in touch' }}</span>
      <h1 class="page-hero__title">{{ $hero?->content['heading'] ?? 'Contact' }}</h1>
      <div class="page-hero__text">{!! $hero?->content['body'] ?? '<p>If you are considering therapy, you are welcome to schedule a free 30-minute introduction call — no commitment required.</p>' !!}</div>
    </div>
  </div>
    ...
    <div class="container">
      <div class="contact-grid">
        ...
        <div>
          ...
          <h2 id="contact-heading" style="margin-bottom:var(--space-8);">{{ $info?->content['heading'] ?? "Let's talk" }}</h2>

          <!-- WhatsApp CTA -->
          @php $waNumber = $info?->content['whatsapp_number'] ?? '66935309052'; @endphp
          <div class="whatsapp-cta">
            <h3>{{ $info?->content['whatsapp_text'] ?? 'Prefer a quick message?' }}</h3>
            <p>Send a WhatsApp message and I'll respond within 24 hours.</p>
        ...
        <div>
          ...
          <div class="booking-cta-card">
            ...
            <p>{!! $booking?->content['body'] ?? "Schedule a no-commitment introduction to see if we're a good fit. Primarily online, with limited in-person availability in Amsterdam." !!}</p>
