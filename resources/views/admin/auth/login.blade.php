...
<head>
  ...
  <title>Admin Login | Therapist Lysander</title>
...
<body>
  <div class="login-card">
    ...
    <p class="login-card__sub">Sign in to manage your website content.</p>
    ...
    @if(session('success'))
      <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:var(--radius);padding:var(--space-3) var(--space-4);font-size:var(--size-sm);color:#166534;margin-bottom:var(--space-6);">{{ session('success') }}</div>
    @endif
    ...
    <form method="POST" action="{{ route('admin.login.submit') }}">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-top:var(--space-4);margin-bottom:var(--space-6);">
        <div style="display:flex;align-items:center;gap:var(--space-2);">
          <input type="checkbox" id="remember" name="remember" style="width:16px;height:16px;">
          <label for="remember" style="font-size:var(--size-sm);color:var(--color-text-muted);">Remember me</label>
        </div>
        <a href="{{ route('admin.password.request') }}" style="font-size:var(--size-sm);color:var(--color-primary);">Forgot password?</a>
      </div>
    ...
    <div style="margin-top:var(--space-6);padding-top:var(--space-6);border-top:1px solid var(--color-border);text-align:center;">
      <a href="{{ route('home') }}" style="font-size:var(--size-sm);color:var(--color-text-muted);">
